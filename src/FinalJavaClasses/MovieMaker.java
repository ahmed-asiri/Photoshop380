...
public class MovieMaker
{
                                  ...
                                  String message)
  {
    ...
    for (int i = 0; i < framesPerSec * 3; i++)
    {
      ...
      g.setFont(font);
	  if (i<=framesPerSec)
      g.drawString(message,300 - (i*10),100);
           if (i>framesPerSec&&i<=framesPerSec*2)
      g.drawString(message,((i-framesPerSec)*10),100);
           if (i>framesPerSec*2&&i<=framesPerSec*3)
      g.drawString(message,(300 -(i-framesPerSec*2)*10),100);
      // add frame to sequencer
      frameSequencer.addFrame(p);
    }
...
//      g.fillRect(i * 10, i * 5, 50,50);
  ...
  public void moveMarksHead(String directory)
  {
    ...
    for (int i = 0; i < framesPerSec; i++)
    {
      ...
      target.copy(markP,281,164,382,301,randomX,randomY);
  ...
  {
    // load the pictures
    String kidF = FileChooser.pickAFile();//"kid-in-frame.jpg"
    Picture kidP = null;
    String wallF = FileChooser.pickAFile();//bgframe.jpg
    Picture wallP = new Picture(wallF);
    String beachF = FileChooser.pickAFile();//beach.jpg
